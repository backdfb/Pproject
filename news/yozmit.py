...
search.title("네이버 뉴스 자동 수집기")
search.geometry("300x50")

display = tk.Entry(search, width=20)
display.pack()

def func(event):
    keyword = display.get()  # 수정된 부분
    print(keyword)  # 검색어 출력
    url = "https://news.google.com/search?q={}".format(keyword)

    try:
        html = requests.get(url)
        html.raise_for_status()  # HTTP 오류가 발생하면 예외를 발생시킴
        soup = BeautifulSoup(html.text, "html.parser")
        myurls = soup.find_all('a', class_='WwrzSb')  # CSS 클래스 확인
